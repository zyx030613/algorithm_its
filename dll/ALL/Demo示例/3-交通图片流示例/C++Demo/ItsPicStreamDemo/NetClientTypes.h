...
#define MAX_PICTURECHANNEL_NUM		32		// The maximum number of picture channels
...
typedef struct
{
    ...
    int             m_iPosition;        //// 0 ~ 100, locate the file playback position; -1, do not locate
...
typedef struct
{
	...
	int				iFileAttr;		//File attributes:0: nvr local storage; 10000: ipc storage
...
typedef struct TDigitalChannelParam
{
	...
	int iConnectMode;						//	[connection] - 0, IP; 1, domain name; 2, active mode, the default is IP
...
#define VCA_EVENT_MAX						(VCA_EVENT_MIN + 33) // invalid event ID, maximum limit
...
#define VCA_CMD_MAX							(VCA_CMD_MIN + 43) // max
...
} vca_TPolygonEx;						//	sizeof = 32*8+4 = 260
...
typedef struct tagWaterPresetInfo
{
	...
	RECT rcGaugeRect; // Diagonal coordinate of the scale area
	RECT rcAssistRect; // auxiliary area diagonal coordinates
	RECT rcGaugeLine; // Coordinate of the ruler reference line
...
typedef struct STORAGE_RULE
{
	...
	int			iType;				// parameter type 0: whether to record audio 1: expiration time (unit: days) 2: whether to open the pumping frame function 3: whether to open the redundant video
...
#define ALARMLINKTYPE_MAX				(ALARMLINKTYPE_MIN + 19)
...
#define  ALARM_TYPE_VIDEO_LOST					 (ALARM_TYPE_MIN + 0)   //Video loss
#define  ALARM_TYPE_PORT_ALARM                   (ALARM_TYPE_MIN + 1)   //Port alarm            
#define  ALARM_TYPE_MOTION_DETECTION			 (ALARM_TYPE_MIN + 2)   //Motion detection
#define  ALARM_TYPE_VIDEO_COVER					 (ALARM_TYPE_MIN + 3)   //Video occlusion
#define	 ALARM_TYPE_VCA							 (ALARM_TYPE_MIN + 4)	//Intelligent analysis
#define  ALARM_TYPE_AUDIOLOST					 (ALARM_TYPE_MIN + 5)	//Audio missing
#define  ALARM_TYPE_TEMPERATURE                  (ALARM_TYPE_MIN + 6)	//Temperature and humidity
#define  ALARM_TYPE_ILLEGAL_DETECT               (ALARM_TYPE_MIN + 7)	//Violation detection
...
#define  ALARM_TYPE_PORT_ALARM_OFF				 (ALARM_TYPE_MIN + 1 + 256)//Port stop alarm
///////////////////////////////////////ExceptionType///////////////////////////////////
...
#define  EXCEPTION_TYPE_MAX						(EXCEPTION_TYPE_MIN + 21)
...
#define MAX_REALTYPE_NUM        2  //Real time data type ，0：Real time brightness value of high definition camera;1：The camera real-time information sharpness
...
#define ITS_EXTRAINFO_CMD_MAX									(ITS_EXTRAINFO_COMMON_CMD_MIN + 39)
...
#define ITS_UNIVERSAL_EABLE_TYPE_MAX						 31
...
#define CMD_NETFILE_QUERY_MAX					(CMD_NETFILE_QUERY_MIN + 8)
...
#define DOWNLOAD_CMD_MAX				(DOWNLOAD_CMD_MIN+7)

#define DOWNLOAD_FILE_FLAG_MULTI		0
#define DOWNLOAD_FILE_FLAG_SINGLE		1	

#define DOWNLOAD_FILE_TYPE_SDV			0
#define DOWNLOAD_FILE_TYPE_MP4			1
#define DOWNLOAD_FILE_TYPE_PS			3
...
typedef struct
{
	...
	int				m_iSaveFileType;		//0:SDV	1:MP4
...
#define NET_CLIENT_MAX								(NET_CLIENT_MIN + 200)
...
#define DOME_PARA_MAX										(DOME_PARA_MIN + 38)		
//
...
#define COMMAND_ID_MAX							(COMMAND_ID_MIN + 76)
...
typedef struct tagFaceDetectArithmetic
{
	...
	int				iPushMode;			//push mode 0:reserved 1:Speed first 2:Quality first  3:custom	
	int				iPushLevel;			//push level effect when push_mode == 3;(0:reserved,1:fast 2:normal 3:slow)
...
//Intelligent analysis rules general parameters
...
typedef struct tagCallParam
{
	...
	int		iType;						//0-Call Scene ID；1-Call privacy shelter location
...
#define  MAX_ITS_OSDTYPE_NUM	       47		//Number of label types
...
#define CMD_DH_ALARM_MAX					(CMD_DH_ALARM_MIN + 11)//MAX
...
typedef struct tagLogonPara
{
	...
	char	cNvsIP[LEN_32];			//IP address，not more than 32 characters, including '\0'
...
typedef  struct WaterSamplePoint
{
	...
	int	iGaugeType;			//Gauge type:0-Reserved, 1-Wide gauge,New gauge, 2-Narrow gauge,Old gauge, 3-Special water ruler, self-calibration
	int iPresetCount;
...
#define	VCA_ARITHMETIC_TYPE_MAX					(VCA_ARITHMETIC_TYPE_MIN+32)	//Arithmetic Type Max
...
#define MAX_MAIN_FUNC_TYPE				(MIN_MAIN_FUNC_TYPE + 0x19)
...
#define ROUTENAT_PORT_MAX					(ROUTENAT_PORT_MIN + 4)
...
#define DSM_CMD_GET_MAX					(DSM_CMD_GET_MIN + 3) 
...
typedef struct tagDsmOnline
{
	int		iSize;
	char	cProductID[LEN_32];	//product id	
	int		iOnline;			//0--Offline 1--Online
	char    cWanIP[LEN_32];
	char    cLanIP[LEN_32];
} DsmOnline;
...
#define MAX_EXDEC_COUNT_BY_TYPE		10		//Maximum number of peripherals per device supported
...
typedef struct tagGetSeekNVSS
{
	...
	char	cKernelVersion[LEN_32];
	char	cOcxVersion[LEN_32];
	char	cGuiVersion[LEN_32];
	char	cSlaveVersion[LEN_32];
	int		iLanNum;
	int		iUpTime;
	int		iCheckCode;
	int		iActivation;
...
#define MAX_WHITE_PLATE_NUM					64	//max white plate num
typedef struct tagITSParkWhiteList
{
	...
	char				cLicensePlateList[MAX_WHITE_PLATE_NUM][MAX_LICENSE_PLATE_LEN];
...
#define MAX_CALIBRATE_POINT_NUM				6
...
typedef struct tagItsPicStream
{
	...
	int	  		iCarColor;				//the color of car
	RECT		tPlateRange;			//License plate range
	int   		iCharNum;				//The number of characters in the license plate
	int   		iCharConfid[MAX_LP_CHAR_COUNT];		//Each character confidence, up to 12 characters
	int   		iPlateConfid;						//The entire license plate confidence
	int   		iRecrdoNum;							//Identify picture serial number
	float 		fSpeed;					//Vehicle speed
	int 		iVehicleDirection;		//Direction of the vehicle
	int			iAlarmType;				//Alarm type
	char  		cCameraIP[LEN_16];		//Camera IP
	...
	int			iSize;					//The size of strcut PicData
	PicData*	ptPicData[MAX_ITS_CAP_PIC_COUNT];
	int			iPlatCount;
	PicData*	ptPlatData;
	int			iFaceCount;
	PicData*	ptFaceData[MAX_ITS_CAP_FACE_COUNT];
	//
	int			iPreset;					
    int			iArea;                			
    char		cFileName[LEN_32];				
    long		lCarSerialNum;					
    int			iPictureNum;					
	int			iPicType;						
	int			iPlatePicLen;					
	int			iFacePicNum;					
	char 		cChannelName[LEN_64];			
	char		cChannelDirection[LEN_64];		
	char		cCrossingID[LEN_64];			
	char		cCrossingName[LEN_64];			
	int			CarRegionNum;					
	RECT		tCarRegion;						
	int			iTargetType;					
	int			iFacePicLen[3];					
	int			iRecordAttr;					
	int 		iLinkageChannelNo;				
	int			iLinkageNo;						
	char		cLinkageCameraIP[LEN_16];		
	int			iVehicleBrand;					
	RECT_S		tFaceRegion;					
	int			iAlgIllgalType;					
	int			iAlarmCode;						
	...
	short		sMergeOSDSize;					
	short		sMergeOSDType;					
	short		sSingleOSDSize;					
	short		sSingleOSDType;					
	short		sOriginalImgWidth;				
	short		sOriginalImgHeight;				
	RECT_S		tVehicleRegion;					
	RECT_S		tCopilotRegion;					
	char  		cCaptureTimeEx2[5][LEN_8];		
	int	  		iCaptureLenEx2[5];				
	char		cUserDefChannelID[LEN_64];		
	
	short		sBodyOfCarLeight;	            
	char		cSecurityCode[LEN_8][LEN_36];	
	int			iSpeedPercent;					
	char		cLinkPanoramaCapUUID[LEN_128];	
	int			iRedBeginTimeMS;				
	int			iRedEndTimeMS;					
	int			iIPDCapType;					
	char		cAlarmCode[LEN_16];
	int 		iCarFeatures[2];
	STRCT_BRAND	stBrandInfo;	
	int 		iMainDriverSex;	
	int  		iCopilpt;		
	int			iNewTargetType;	
	int 		iRealImgWidth;	
...
typedef struct tagFacePicData
{
	...
	int			iDrop;					//The face track ends, 0 is not over, 1 ends
...
#define FACEATTR_TYPE_GENDER			1	//gender
#define FACEATTR_TYPE_MASKS				2	//masks
#define FACEATTR_TYPE_BEARD				3	//beard
#define FACEATTR_TYPE_EYEOPEN			4	//eye open
#define FACEATTR_TYPE_MOUTH				5	//mouth
#define FACEATTR_TYPE_GLASSES			6	//glasses
#define FACEATTR_TYPE_RACE				7	//race
#define FACEATTR_TYPE_EMOTION			8	//emotion
#define FACEATTR_TYPE_SMILE				9	//smile
#define FACEATTR_TYPE_VALUE				10	//value
#define FACEATTR_TYPE_NATION			11	//nation
#define FACEATTR_TYPE_REFID				17	//ref id
#define FACEATTR_TYPE_OBJTYPE			18	//object type
...
#define MAX_USER_FIND_PWD_MODE_NUM		3
...
#define FACE_CMD_MAX								(FACE_CMD_MIN + 0x10)	


#define FACE_MAX_PAGE_COUNT							20
#define FACE_MAX_KEY_COUNT							32
...
typedef struct tagFaceModeling
{
	...
	int					iLibKey;
	int					iFaceNum;
	int					iFaceKey[FACE_MAX_PAGE_COUNT];
